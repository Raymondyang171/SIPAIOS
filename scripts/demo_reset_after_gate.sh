...
info "Next steps:"
echo ""
echo "1. RESTART API SERVER (single command with auto-verification):"
echo "   ./scripts/dev_restart_api.sh"
echo ""
echo "2. Open http://localhost:3002/purchase/orders/create in browser"
echo "   - Verify Items and UOM dropdowns have values"
echo "   - If dropdowns are empty, check: login status, company context"
echo ""
echo "Note: Run this script AFTER gate_app02.sh, not before."
echo "Correct order: gate_app02.sh -> demo_reset_after_gate.sh -> dev_restart_api.sh -> open UI"
