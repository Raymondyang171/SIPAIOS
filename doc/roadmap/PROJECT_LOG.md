...
Title：15 app 06 B:14 SVC-APP-006-B 完成：Production MO Seed + Newman 覆蓋（BOM Version Lock）
...
Scope Impact：API / DB / Ops / Docs

Related SVC：SVC-APP-006-B

Next Owner：You / Claude(VSCode) / ChatGPT(Web)

Tags（可選）：app-03, production-mo, newman, seed, gate, bom-version-lock, tenant-isolation

結論（已定案）

SVC-APP-006-B 已完成並可重播驗收：新增 Production MO 測試 seed，並以 Newman（Postman collection）補齊 APP-03 覆蓋，BOM Version Lock 行為以測試證據鏈固化。

./scripts/gate_app02.sh 全流程 PASS（含 DB replay、Seeds、Newman），Newman 結果 20 requests / 51 assertions / 0 failures。

變更已 commit 並 push 至 origin/main（commit：0daf277）。

本次確定方案邊界取捨

不修改 MO API handler：因既有 bom_version_id 驗證邏輯已存在（以證據指紋確認），改以「seed + 測試」完成最小交付。

驗收以 gate/newman 為唯一可信輸出，不新增 UI、不啟動 APP-007 backflush。

暫接受將 APP-03 Production MO 測試掛載於 gate_app02.sh（先確保可重播與回歸 PASS，治理優化後置）。

未決事項

是否拆分獨立 gate_app03.sh（治理清晰度提升，建議列入後續但不阻擋）。

Newman 執行時出現 Node DeprecationWarning（DEP0176：fs.F_OK）根因追查與修復時機（建議列 tech debt）。

下一步（1–3 個最小動作）

新開對話啟動 SVC-APP-007（Backflush for MO）：先做 Step0 證據定位與驗收錨點定義（扣料規則/倉別/UOM/負庫策略/跨租防線）。

（可選）將 APP-03 Gate 從 gate_app02.sh 拆出獨立腳本並更新 Roadmap/Log 以對齊責任邊界。

驗收錨點（可觀測結果）

Git 交付：main -> origin/main 已同步，commit 0daf277。

Gate 重播：./scripts/gate_app02.sh → [GATE PASS]，Stage2C replay PASS、Seeds 001/002/003 全部成功。

Newman：20 requests / 51 assertions / 0 failures；APP-03 覆蓋含：

Create MO 成功（201，回傳含 bom_version_id locked）

Get MO 驗證 bom_version_id 一致

Missing bom_version_id → 400（VALIDATION_ERROR）

Invalid / Cross-tenant bom_version_id → 403（FORBIDDEN）

List MOs → 200 且含 bom_version_id

風險與防線 / 變更紀錄

風險：APP-02 Gate 內含 APP-03 測試，後續維運可能誤解 Gate 職責；防線：後續拆 gate_app03.sh，讓 Gate 與模組責任對齊。

風險：DeprecationWarning 噪音可能掩蓋真正警告；防線：列入 tech debt，待 Node/依賴升級窗口集中處理。

變更紀錄（4 檔，白名單內）：

新增 apps/api/seeds/003_production_mo_data.sql

修改 apps/api/postman/SIP-AIOS-Auth.postman_collection.json

修改 apps/api/postman/SIP-AIOS-Local.postman_environment.json

修改 scripts/gate_app02.sh（納入 003 seed 執行）
