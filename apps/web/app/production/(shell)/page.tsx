...
export default function ProductionHomePage() {
  ...
  const menuItems = [
    { title: "Work Orders", description: "Manage manufacturing orders", icon: "W", href: "/production/work-orders" },
    { title: "Production Report", description: "View production metrics", icon: "R", href: null },
    { title: "Inventory", description: "Check stock levels", icon: "I", href: null },
    { title: "Settings", description: "Configure system options", icon: "S", href: null },
  ];
          ...
          {menuItems.map((item) => {
            const cardContent = (
              <div className="flex items-start gap-4">
                <span className="w-8 h-8 flex items-center justify-center rounded bg-zinc-200 dark:bg-zinc-600 text-sm font-bold text-zinc-700 dark:text-zinc-200">
                  {item.icon}
                </span>
                <div>
            ...
            const cardClassName = "rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-5 hover:border-zinc-300 dark:hover:border-zinc-600 transition-colors cursor-pointer block";
            ...
            return item.href ? (
              ...
              <div key={item.title} className={`${cardClassName} opacity-60 cursor-not-allowed`}>
                {cardContent}
              </div>
            );
          })}
